...
class CagarAlamController extends Controller
{
    ...
    {
        $jenis_hutan = JenisHutan::find(3);

        $data_luas = LuasHutan::select('*', DB::raw('SUM(luas) as luass'))
            ->where('jenis_hutan_id', '=', 3)
            ->groupBy(DB::raw("YEAR(tanggal)"))
            ->join('jenis_hutan', 'jenis_hutan.id', '=', 'luas_hutan.jenis_hutan_id')
            ->get();

            $tahun_awal = LuasHutan::where('jenis_hutan_id', '=', 3)->min('tanggal');
            $tahun_akhir = LuasHutan::where('jenis_hutan_id', '=', 3)->max('tanggal');
